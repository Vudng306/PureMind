    ...
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-20 border-b border-line bg-bg">
        <div className="flex h-[72px] items-center gap-3 px-4 sm:gap-7 sm:px-10">
          ...
          >
            <Icon name="search" />
            <span className="flex-1 text-left">Tìm tài liệu, highlight…</span>
            <kbd className="rounded-[5px] border border-line px-1.5 py-0.5 font-sans text-xs">Ctrl K</kbd>
          </button>
          <button type="button" className="icon-btn lg:hidden" aria-label="Tìm nhanh" onClick={() => setPaletteOpen(true)}>
            <Icon name="search" />
          </button>
          ...
          <Link href="/settings" aria-label="Tài khoản và cài đặt" className="rounded-full">
            {user ? (
        ...
        <nav aria-label="Điều hướng chính (di động)" className="flex gap-1 px-4 pb-3 md:hidden">
          {NAV.map((item) => (
